...
class Camera(views.APIView):
    ...
    @swagger_auto_schema(operation_description="Add a new device",manual_parameters=get_header_params(),request_body=serializers.DeviceSerializer,responses={201:serializers.DeviceSerializer(many=False)})
    ...
    @swagger_auto_schema(operation_description="Get all devices of the user",manual_parameters=get_header_params(),responses={200:serializers.DeviceSerializer(many=True)})
    ...
    @swagger_auto_schema(operation_description="Delete a device",manual_parameters=get_header_params(),request_body=serializers.DeviceSerializerForDelete,responses={200:"Device deleted successfully!",400:"Device does not exist!"})
    ...
    @swagger_auto_schema(operation_description="Update a device",manual_parameters=get_header_params(),request_body=serializers.DeviceSerializer,responses={200:"Device updated successfully!",400:"Device does not exist!"})
    def put(self, request):
        print(self.request.data)
        # check if the device belongs to the user
        patient = Patient.objects.get(userid=self.request.user.id)
        device = Devices.objects.filter(deviceid=self.request.data['deviceId'],accountid=patient)
        if not device:
            return Response({'message':"Device does not exist!"},status=status.HTTP_400_BAD_REQUEST)
        device.update(companyname=self.request.data['companyName'], ipaddress=self.request.data['ipAddress'], 
                                   password=self.request.data['password'],username=self.request.data['userName'],
                                   rtspport=self.request.data['rtspPort'],channel=self.request.data['channel'])
